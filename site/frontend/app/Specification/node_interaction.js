import { line_data ,poi_colordomain} from '../Specification/Node.js'
    ...
    if(((dx+prex)<=0))
    ...
    else
     {
        ...
        let current_id=d3.select(condition_node).attr('id')
         let current_location= nodelist.order.indexOf(current_id)
        let next_node=null;
...
export  function drag_end(){
    ...
    d3.select('#'+nodelist.order[current_location])
        .style('left', current_location*(parseInt(d3.select(condition_node).style('width'))+22)+"px");
    d3.select(condition_node).style('z-index',0);
}

//切换关键词是否显示，并对相应的连线进行刷新
...
//关键词那栏show，hide或滚动时都更新左侧节点位置，用于更新线的位置
