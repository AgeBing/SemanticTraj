...
import { coorHexagonCopy,SVGlength,iconSrcUrl }  from './config.js'
    	...
    	let item = container
   	 	...
   	 	item.append('div').attr('class','node-legend-rect')
				...
				switch(topicNames[i].name){
					case "住宅":
						return `url("${iconSrcUrl}Residential.png")`
					break;
					case "娱乐商业":
						return `url("${iconSrcUrl}Entertainment.png")`
					break;
					case "办公":
						return `url("${iconSrcUrl}Business.png")`
					break;
					case "医疗":
						return `url("${iconSrcUrl}Medical.png")`
					break;
					case "交通":
						return `url("${iconSrcUrl}Traffic.png")`
					break;
					case "教育":
						return `url("${iconSrcUrl}Educational.png")`
					break;
				}
   	 		})
   	 })
